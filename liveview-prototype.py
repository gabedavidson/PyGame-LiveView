...
font_large = pg.freetype.SysFont('arialblack', 30, italic=False, bold=False)
font_med = pg.freetype.SysFont('arialblack', 22, italic=False, bold=False)
font_small = pg.freetype.SysFont('arialblack', 15, italic=False, bold=False)
...
class Environment:
    def __init__(self, name, default_zones_across: int = 5, default_zones_down: int = 5, border_width: int = 3,
        ...
        self.default_zone_width, self.default_zone_height = _env_screen_size[0] // self.active_zones_across, _env_screen_size[1] // self.active_zones_down
        self.zone_width, self.zone_height = self.default_zone_width, self.default_zone_height
    ...
    def compile_env_borders(self):
        self.borders = []
        print(self.active_zones_across, self.active_zones_down)
    ...
    def resize_tiles(self):
        ...
        for zone in self.zones:
            for zvp in zone.pool:
                for var in zvp.vars:
                    var.data = (var.data[0], var.data[1], var.data[2] * ratio_x, var.data[3] * ratio_y)

    def reposition_tiles(self):
        for _across, zone_id in enumerate(self.active_zones):
            for zvp in self.zones[zone_id].pool:
                for var in zvp.vars:
                    var.data = (var.data[0] / self.env.default_zone_width * self.env.zone_width,
                                var.data[1] / self.env.default_zone_height * self.env.zone_height,
                                var.data[2], var.data[3])
...
class _States:
    def __init__(self, _states: list[str], env: list[Environment] | Environment):
        self.states = {_state: StateZonePool([Zone(id=i, top_left=(Helper.zid_to_coords(env[_i] if isinstance(env, list) else env, i, True)), pool=[]) for i in range(env[_i].zone_count)], env=env[_i] if isinstance(env, list) else env, active_zones=[_zid for _zid in range(env[_i].zone_count if isinstance(env, list) else env.zone_count)]) for _i, _state in enumerate(_states)}
        self.env = None
    ...
    def _add(self, name: str, env: Environment, override=False, catch=True):
        ...
        self.states[name] = StateZonePool([Zone(id=i, top_left=(Helper.zid_to_coords(self.env, i, _screen_coords=True)), pool=[]) for i in range(self.env.zone_count)], env=env, active_zones=[_zid for _zid in range(env.zone_count)])
...
class Simulation:
    ...
    def draw_state(states: _States):
        ...
        svp = states[Simulation.curr_state]
        for zone_id in svp.active_zones:
            for zvp in svp.zones[zone_id].pool:
                font_small.render_to(screen, (svp.zones[zone_id].top_left[0] + 5, svp.zones[zone_id].top_left[1] + 5), str(svp.zones[zone_id].id), (0, 0, 0))
    ...
    @staticmethod
    def update_env_dimensions(states: _States, active_zones_across_inset=0, active_zones_down_inset=0, active_zones_across=-1, active_zones_down=-1):
        # Change the dimensions of the environment –> update env variables
        states[Simulation.curr_state].env.change_env_dimensions(active_zones_across_inset, active_zones_down_inset, active_zones_across, active_zones_down)
        # Change active zones –> after changing env variables
        Simulation.update_active_zones(states)
        # Change tile rects to scale and position dynamically with the new dimensions
        states[Simulation.curr_state].resize_tiles()
        states[Simulation.curr_state].reposition_tiles()

    @staticmethod
    def update_active_zones(states: _States):
        print('changing active zones. is currently:', states[Simulation.curr_state].active_zones)
        states[Simulation.curr_state].active_zones = []
        for _a in range(states[Simulation.curr_state].env.zoom_inset[0], states[Simulation.curr_state].env.zoom_inset[0] + states[Simulation.curr_state].env.active_zones_across):  # fixme, so that this happens once when state is changed or environment is changed.
            for _d in range(states[Simulation.curr_state].env.zoom_inset[1], states[Simulation.curr_state].env.zoom_inset[1] + states[Simulation.curr_state].env.active_zones_down):
                states[Simulation.curr_state].active_zones.append(Helper.zcoords_to_zid(states[Simulation.curr_state].env, [_a, _d]))
        print('active zones is now:', states[Simulation.curr_state].active_zones)
...
my_env_1 = Environment('env1', border_width=5, border_color=COLORS.RED)
my_env_2 = Environment('env2', default_zones_across=3, default_zones_down=3, border_color=COLORS.DGRAY)
...
while running:
    for event in pg.event.get():
        ...
        if event.type == pg.KEYDOWN:
            if not Simulation.user_input_active:
                ...
                if event.key == pg.K_q:
                    Simulation.update_env_dimensions(States, 1, 1, 2, 4)
                elif event.key == pg.K_w:
                    Simulation.update_env_dimensions(States)
            else:
    ...
    pg.display.update()
    print(States[Simulation.curr_state].active_zones)
