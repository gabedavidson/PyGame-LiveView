...
class Environment:
    def __init__(self, dza: int = 5, dzd: int = 5, b_width: int = 3, b_color: Color = COLORS.BLACK, def_reso: int = 5, def_color: Color = COLORS.BLACK):
        self.default_zones_across, self.default_zones_down = dza, dzd
        ...
        self.default_resolution = def_reso
    ...
    def change_env_dimensions(self, _aza_offset, _azd_offset, _aza, _azd):
        assert 0 < _aza <= self.default_zones_across and 0 < _azd <= self.default_zones_down
        self.active_zones_across = _aza if self.default_zones_across - _aza_offset >= _aza else self.default_zones_across - _aza_offset
        self.active_zones_down = _azd if self.default_zones_down - _azd_offset >= _azd else self.default_zones_down - _azd_offset
        ...
        self.zoom_offset = [_aza_offset, _azd_offset]
...
class _States:
    ...
    def change_env_dimensions(self, x_offset, y_offset, _w, _h):
        ...
        for i, z in enumerate(self[Simulation.curr_state].zones):
            good_across = self.env.zoom_offset[0] <= z.id % self.env.default_zones_across < min(self.env.default_zones_across, self.env.zoom_offset[0] + self.env.default_zones_across)
            good_down = self.env.zoom_offset[1] <= z.id // self.env.default_zones_across < min(self.env.default_zones_down, self.env.zoom_offset[1] + self.env.default_zones_down)
...
class Simulation:
    ...
    def draw_state(states: _States, dynamic_sizing: bool = True, dynamic_position: bool = True):
        ...
        for _a in range(states[Simulation.curr_state].env.zoom_offset[0], states[Simulation.curr_state].env.zoom_offset[0] + states[Simulation.curr_state].env.active_zones_across):  # fixme, so that this happens once when state is changed or environment is changed.
            for _d in range(states[Simulation.curr_state].env.zoom_offset[1], states[Simulation.curr_state].env.zoom_offset[1] + states[Simulation.curr_state].env.active_zones_down):
                _zone = states[Simulation.curr_state].zones[Helper.zcoords_to_zid(states[Simulation.curr_state].env, [_a, _d])]
                _update_tile_data(_zone)
    ...
    def draw_env_borders(states: _States):
        print(states[Simulation.curr_state].env.borders)
...
my_env_1 = Environment(b_width=5, b_color=COLORS.RED)
my_env_2 = Environment(dza=3, dzd=3, b_color=COLORS.DGRAY)
...
#     pg.draw.rect(screen, COLORS.BLACK.color())

    # States.env.a__change_za_yd(1, 1, 2, 5)
